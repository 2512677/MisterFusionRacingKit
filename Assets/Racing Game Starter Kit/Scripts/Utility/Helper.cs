        ...
        public static AudioSource CreateAudioSource(GameObject go, AudioClip clip, string audioMixerGroup, float spatialBlend, float volume, bool loop, bool playOnStartup)
        {
            ...
            if (GlobalSettings.Instance != null)
            {
				...
				if (mixer != null && audioMixerGroup != "")
                {
                    ...
                    {
                        Debug.LogWarning("Группа звукорежиссеров '" + audioMixerGroup + "' не найден в Audio Mixer");
                    }
                }
            }
            else
            {
                Debug.LogWarning(" AudioSource '" + audioSource.name + "' не была назначена группа выходного микшера, поскольку игровой  Audio Mixer не был найден.");
            }
