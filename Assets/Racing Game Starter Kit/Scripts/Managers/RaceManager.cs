...
namespace RGSK
{
    public class RaceManager : MonoBehaviour
    {
        ...
        private void ConfigureChaseRace()
        {
            ...
            // Устанавливаем один круг (если требуется)
            //lapCount = 1;
            opponentCount = 1;



            // Отключаем отображение имён гонщиков
            // showRacerNames = false;

            // Отключаем принудительный респаун при движении в неправильном направлении
            //forceWrongwayRespawn = false;

            // Настройка целей погони
            foreach (var racer in racerList)
            {
                if (racer != playerStatistics) // Все гонщики, кроме игрока
                {
                    // racer.IsTarget = true; // Отметить AI как цели
                }
            }

            //Debug.Log("Настройки для гонки 'Погоня' применены.");
        }


        public void CaptureBot()
        {
            capturedBots++;
            CheckChaseRaceStatus();
        }

        private void CheckChaseRaceStatus()
        {
            if (capturedBots >= opponentCount)
            {
                EndRace(true); // Победа: все боты пойманы
            }
            // else if (currentLap > lapCount)
            {
                EndRace(false); // Поражение: круги закончились
            }
        }
        ...
        private void EndRace(bool playerWon)
        {
            if (playerWon)
            {
                //Debug.Log("Вы поймали всех нарушителей! Победа!");
            }
            else
            {
                //Debug.Log("Время истекло! Вы проиграли.");
            }

            // Логика завершения гонки (UI, перезапуск и т. д.)
        }
        ...
        public void RegisterRacerFinish(RacerStatistics racer)
        {
            ...
            if (racer == playerStatistics)
            {
                ...
                // 7. Отображаем сообщение о месте
                if (RacePanel.instance != null)
                {
                    string localizedMessage = LocalizationManager.GetTranslation("RaceMessages/RacePositionMessage");
                    if (string.IsNullOrEmpty(localizedMessage) || !localizedMessage.Contains("{0}"))
                    {
                        localizedMessage = "Вы заняли {0} место"; // Значение по умолчанию
                    }

                    try
                    {
                        string formattedMessage = string.Format(localizedMessage, Helper.AddOrdinal(playerPosition));
                        RacePanel.instance.ShowRaceEndMessage(formattedMessage);
                    }
            ...
            else
            {
                ...
                if (RacePanel.instance != null)
                {
                    ...
                    {
                        localizedInfo = "{0} занял {1} место"; // по умолчанию
                    }

                    try
                    {
                        string formattedInfo = string.Format(localizedInfo, racerName, Helper.AddOrdinal(position));
                        RacePanel.instance.ShowRaceInfoMessage(formattedInfo);
                    }
                    catch (FormatException ex)
                    {
                        RacePanel.instance.ShowRaceInfoMessage($"{racerName} завершил гонку.");
                    }
                }
            }
