...
namespace RGSK
{
    public class RacerStatistics : MonoBehaviour
    {
        ...
        void WrongwayChecker()
        {
            ...
            float angle = raceDistanceTracker.eulerAngles.y - transform.eulerAngles.y;
            wrongway = Mathf.Abs(angle) <= 230f && Mathf.Abs(angle) >= 120;
        ...
        void RegisterCheckpoint(int index, float time, float dist)
        {
            ...
            // Добавить время к круговому времени гонщика
            checkpointTimer += time;

            if (isPlayer)
            {
                // Показать информацию о контрольной точке
                if (RacePanel.instance != null)
                {
                    string info = "+ " + time;
        ...
        void RegisterSpeedtrap(int index, float dist)
        {
            ...
            if (rigid != null)
            {
                ...
                if (isPlayer)
                {
                    ...
                    if (RacePanel.instance != null)
                    {
                        string info = "+" + speed.ToString("F1") + unit.ToString();
                        RacePanel.instance.ShowRaceInfoMessage(info);
                    }
                }
            }
